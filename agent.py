...
class Agent():
    def __init__(self, action_dim, obs_dim, eps=1, gamma=0.99, decay=0.995, eps_low = 0.05, batch_size=128, tau=0.05, train_mode=True, duel=False):
        self.policy_network = DQN(action_dim, obs_dim) if not duel else DuelDQN(action_dim, obs_dim)
        self.target_network = DQN(action_dim, obs_dim) if not duel else DuelDQN(action_dim, obs_dim)
...
class DQNAgent(Agent):
    def __init__(self, action_dim, obs_dim, eps=1, gamma=0.99, decay=0.995, eps_low = 0.05, batch_size=128, tau=0.05, train_mode=True, duel=False):
        super().__init__(action_dim, obs_dim, eps, gamma, decay, eps_low, batch_size, tau, train_mode, duel)

    def step(self, obs):
        with torch.no_grad():
            action_scores = self.policy_network.forward(torch.from_numpy(obs))
    ...
    def train(self, replay_buffer):
        ...
        Q_local = self.policy_network(obs).gather(1, actions.reshape(-1, 1))
...
class DDQNAgent(Agent):
    def __init__(self, action_dim, obs_dim, eps=1, gamma=0.99, decay=0.995, eps_low=0.05, batch_size=128, tau=0.05, train_mode=True):
    ...
    def train(self, replay_buffer):
        ...
        policy_actions = torch.max(self.policy_network(new_obs), dim=1)[1]
...
class ReplayBuffer:
    ...
    def sample_batch(self, batch_size):
        ...
        obs = torch.Tensor(np.array([experience[0] for experience in mini_batch]))
        ...
        next_obs = torch.Tensor(np.array([experience[3] for experience in mini_batch]))
